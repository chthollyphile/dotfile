...
Panel {
  ...
  // Gateway configuration is separate from `info.gateway`: the latter is the
  // route currently in use, while these fields describe the NetworkManager
  // profile's automatic/manual policy.
  property string gatewayMode: "auto"
  property string configuredGateway: ""
  property string gatewayInput: ""
  property string gatewayError: ""
  property string gatewayTarget: ""
  property string gatewayTargetInterface: ""
  property bool gatewayBusy: false
  readonly property string gatewayScript: String(Qt.resolvedUrl("gateway.sh")).replace(/^file:\/\//, "")
  ...
  function refresh(scanWifi) {
    ...
    if (!gatewayProc.running) gatewayProc.running = true
    if (!dnsProc.running) {
  ...
  function updateGateway(raw) {
    var next = Model.parseKeyValue(raw)
    var mode = next.mode || "auto"

    gatewayMode = mode === "manual" ? "manual" : mode === "auto" ? "auto" : "unavailable"
    configuredGateway = next.configured_gateway || ""
    gatewayTarget = next.target_profile || ""
    gatewayTargetInterface = next.target_interface || ""
    if (!gatewayField.activeFocus && !gatewayBusy) {
      gatewayInput = configuredGateway || next.target_gateway || ""
    }
  }
  ...
  function setBand(band) {
    if (!band || actionProc.running || gatewayActionProc.running) return
  ...
  function setDns(provider) {
    if (!root.bar || !provider || actionProc.running || gatewayActionProc.running) return
  ...
  function setGateway(value) {
    if (!root.info.iface || !gatewayTargetInterface || gatewayMode === "unavailable" || gatewayBusy || actionProc.running) return

    var requested = String(value || "").trim()
    if (!Model.isValidIpv4(requested)) {
      gatewayError = "Enter a valid IPv4 gateway."
      return
    }

    gatewayInput = requested
    gatewayError = ""
    gatewayBusy = true
    gatewayActionProc.command = ["bash", root.gatewayScript, "set", requested]
    gatewayActionProc.running = true
  }

  function setGatewayAuto() {
    if (!root.info.iface || !gatewayTargetInterface || gatewayMode === "unavailable" || gatewayBusy || actionProc.running) return

    gatewayError = ""
    gatewayBusy = true
    gatewayActionProc.command = ["bash", root.gatewayScript, "auto"]
    gatewayActionProc.running = true
  }

  function gatewayStatusText() {
    if (gatewayError !== "") return gatewayError
    if (gatewayBusy) return "Applying gateway…"
    if (gatewayMode === "unavailable") return "No editable NetworkManager uplink"
    if (gatewayTargetInterface !== "" && info.iface !== "" && gatewayTargetInterface !== info.iface) {
      return "Editing " + (gatewayTarget || gatewayTargetInterface) + " · route via " + info.iface
    }
    return gatewayMode === "manual" ? "Manual default route" : "Automatic gateway from DHCP"
  }
  ...
  Process {
    id: gatewayProc
    command: ["bash", root.gatewayScript, "status"]
    stdout: StdioCollector {
      waitForEnd: true
      onStreamFinished: root.updateGateway(text)
    }
  }
  ...
  Process {
    id: gatewayActionProc
    stdout: StdioCollector { id: gatewayActionStdout; waitForEnd: true }
    stderr: StdioCollector { id: gatewayActionStderr; waitForEnd: true }
    onExited: function(exitCode) {
      gatewayBusy = false
      if (exitCode === 0) {
        gatewayError = ""
      } else {
        var message = String(gatewayActionStderr.text || "").trim()
        gatewayError = message || "Gateway change failed; previous settings were restored."
      }
      root.refresh(false)
    }
  }
  ...
  KeyboardPanel {
    ...
    Column {
      ...
      // Gateway policy: keep the live route above, and make the
      // NetworkManager profile's automatic/manual choice editable.
      PanelSeparator {
        visible: !!root.info.iface
        foreground: root.bar.foreground
      }
      ...
      Column {
        ...
        RowLayout {
          ...
          TextField {
            ...
            placeholderText: "IPv4 gateway"
            font.family: Style.font.family
            font.pixelSize: Style.font.body
            foreground: root.bar.foreground
            horizontalPadding: Style.spacing.controlGap
            verticalPadding: Style.spacing.controlPaddingY
            enabled: !root.gatewayBusy
            text: root.gatewayInput
            onTextChanged: if (activeFocus && text !== root.gatewayInput) root.gatewayInput = text
            onAccepted: root.setGateway(text)
          }

          Button {
            text: root.gatewayBusy ? "…" : "Apply"
            foreground: root.bar.foreground
            fontFamily: root.bar.fontFamily
            ...
            bordered: true
            enabled: !root.gatewayBusy && !!root.info.iface && root.gatewayMode !== "unavailable"
              && !!root.gatewayTargetInterface && Model.isValidIpv4(root.gatewayInput)
            onClicked: root.setGateway(root.gatewayInput)
          }

          Button {
            text: "DHCP"
            selected: root.gatewayMode === "auto"
            foreground: root.bar.foreground
            fontFamily: root.bar.fontFamily
            fontSize: Style.font.bodySmall
            horizontalPadding: Style.space(8)
            verticalPadding: Style.space(4)
            bordered: true
            enabled: !root.gatewayBusy && !!root.info.iface && root.gatewayMode !== "auto"
              && root.gatewayMode !== "unavailable" && !!root.gatewayTargetInterface
            onClicked: root.setGatewayAuto()
          }
        }

        Text {
          width: parent.width
          text: root.gatewayStatusText()
          color: root.gatewayError !== "" ? root.bar.urgent : Qt.darker(root.bar.foreground, 1.4)
          font.family: root.bar.fontFamily
          font.pixelSize: Style.font.caption
          elide: Text.ElideRight
        }
